...
global predicted_aspect
if 'predicted_aspect' not in globals():
    predicted_aspect = []
...
#             rating = element[2]
    ...
    def predict_best_aspect_for_given_restaurant(self,item):
        maxi=-99999
        global aspect_values
        if 'aspect_values' not in globals():
            aspect_values = []
        for aspect in range(self.num_aspects):
            avg_sum=0
            length=0
            for user in self.profile_users:
                avg_sum+=self.calculate_aspect_impacts(user,item,average=True,absolute=False)[aspect]
                length+=1
            avg_sum=avg_sum/length
            #print(avg_sum)
            aspect_values+=[(aspect,avg_sum)]
            if  avg_sum>maxi:
                maxi=avg_sum
                max_aspect=aspect
        aspect_values.sort(key=lambda e: e[1])    
        print(str(max_aspect)+' is the best aspect for given restaurant.')
        #print(aspect_values)
        return max_aspect        
...
if __name__ == '__main__':
                    ...
                    for k,v in valuess.items():
                        for n,i in enumerate(arr2):
                            if (k==i and int(v)>=0):
                                ...
                                rew_bus[0]=data2[x]['business_id']
                                rew_bus[1]=data2[x]['review_id']
                                if (data2[x]['stars']>3):
                                    temparr[2]=1
                                else:
                                    temparr[2]=0
    ...
    id_bus=modelnew.predict_best_rest_for_given_aspect(int(z))
    reviews=[]
    for x in range(len(data2)):
    ...
    for x in range(len(label_graph)):
        for user in modelnew.profile_users:
            if(label_graph[x]==user):
                value_avg1.append(modelnew.logistic(modelnew.calculate_sentiment_utility_prediction(user,id_bus)[:-1])[z])
    ...
    sc1=plt.scatter(label_graph,value_graph,color='g',label='a')
    sc1=plt.scatter(label_graph,value_avg1,color='r',label='b')
    plt.tight_layout()
    plt.show()
