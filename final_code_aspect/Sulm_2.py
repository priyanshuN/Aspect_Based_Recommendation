...
#             rating = element[2]
            ...
            message = str(element) + '\nRating: %d\tPrediction: %.3f'%(element[2],r_logistic_prediction)
            message += '\nReal sentiments: %s\nPredicted sentiments: %s'%(element[3:],s_logistic_predictions[:-1])
            if r_logistic_prediction>maxi:
                maxi=r_logistic_prediction
                max_item=item
        return max_item
    ...
    def predict_least_aspect_for_given_restaurant(self,item):
        mini=99999
        ...
        for aspect in range(self.num_aspects):
            ...
            if  avg_sum<mini:
                mini=avg_sum
                min_aspect=aspect
        print(str(min_aspect)+' is the least aspect for given restaurant.')
        return min_aspect
...
if __name__ == '__main__':
    ...
    modelnew.predict_best_rest_for_given_aspect(int(aspect_input))


    restaurant_input=input('Enter the restaurant ')
    modelnew.predict_best_aspect_for_given_restaurant(str(restaurant_input))

    restaurant_input=input('Enter the restaurant ')
    modelnew.predict_least_aspect_for_given_restaurant(str(restaurant_input))
